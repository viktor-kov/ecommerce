        ...
        <section class="bg-gray-200 p-4">
            <form action="../editproduct/{{$product->id}}" method="post" enctype="multipart/form-data">
                ...
                <p class="flex justify-between">Názov
                    <input type="text" name="product_name" id="" class="w-3/4" value="{{$product->name}}">
                </p>
                <p class="flex justify-between mt-1">Popis
                    <textarea name="product_description" rows="10" class="w-3/4">{{$product->text}}</textarea>
                </p>
                ...
                <p class="flex justify-between mt-1">Cena
                    <input type="text" name="product_price" id="" class="w-3/4" value="{{$product->price}}"">
                </p>
                <p>
                ...
                <p class="mt-2">
                    <input type="submit" value="Upraviť produkt" class="p-4 bg-green-400 text-white w-full cursor-pointer">
                </p>
            </form>
            <form action="../delete/{{$product->id}}" method="post">
                @csrf
                <input type="hidden" name="product_photo_path" value="{{$product->photo_path}}">
                <input type="submit" value="Zmazať produkt" class="p-4 bg-red-600 text-white w-full cursor-pointer mt-4">
            </form>
            <section class="p-4">
