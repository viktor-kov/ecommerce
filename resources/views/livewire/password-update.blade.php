    ...
    <form wire:submit.prevent="passwordUpdate" action="#" method="POST">
        ...
        <aside class="mt-4">
            <p class="md:flex md:justify-between m-1">
                <span class="break-words">{{__('profile.old-pwd')}}</span> <input type="password" name="current_password" class="w-full md:w-3/4 border border-black p-1" wire:model.defer="current_password">
            </p>
            @error('current_password') <span class="text-red-500">{{ $message }}</span> @enderror
            <p class="md:flex md:justify-between m-1">
                <span class="break-words">{{__('profile.new-pwd')}}</span> <input type="password" name="new_password" class="w-full md:w-3/4 border border-black p-1" wire:model.defer="new_password">
            </p>
